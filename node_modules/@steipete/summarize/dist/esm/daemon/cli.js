...
async function waitForHealth({ fetchImpl, port, timeoutMs, }) {
...
async function waitForHealthWithRetries({ fetchImpl, port, attempts, timeoutMs, delayMs, }) {
    let lastError = null;
...
async function checkAuth({ fetchImpl, token, port, }) {
    try {
...
async function checkAuthWithRetries({ fetchImpl, token, port, attempts, delayMs, }) {
    for (let attempt = 0; attempt < attempts; attempt += 1) {
...
async function resolveCliEntrypointPathForService() {
    ...
    throw new Error(`Cannot find built CLI at ${distCandidates.join(' or ')}. Run "pnpm build:cli" (or "pnpm build") first, or pass --dev to install a dev daemon.`);
}
function resolveRepoRootForDev() {
    ...
    if (srcIndex === -1)
        throw new Error('Dev mode requires running from repo (src/cli.ts)');
...
async function resolveDaemonProgramArguments({ dev, }) {
...
function formatProgramArguments(args) {
    return args
        .map((arg) => {
        if (!/[\s"]/g.test(arg))
            return arg;
...
async function readInstalledDaemonCommand(env) {
    if (process.platform === 'darwin')
        return readLaunchAgentProgramArguments(env);
    if (process.platform === 'linux')
        return readSystemdServiceExecStart(env);
    if (process.platform === 'win32')
        return readScheduledTaskCommand(env);
    return null;
}
export async function handleDaemonRequest({ normalizedArgv, envForRun, fetchImpl, stdout, stderr, }) {
    if (normalizedArgv[0]?.toLowerCase() !== 'daemon')
        return false;
    ...
    if (sub === 'install') {
        ...
        if (!authed)
            throw new Error('Daemon is up but auth failed (token mismatch?)');
    ...
    if (sub === 'restart') {
        ...
        const loaded = await service.isLoaded({ env: envForRun });
        if (!loaded) {
            stdout.write(`${service.label} ${service.notLoadedText}. Run: summarize daemon install --token <token>\n`);
            return true;
        }
        ...
        const authed = healthy
            ...
            : false;
        if (!healthy || !authed) {
            stdout.write('Restarted daemon. It is still starting; run "summarize daemon status" in a few seconds.\n');
            return true;
        }
    ...
    if (sub === 'uninstall') {
        ...
        stdout.write('Uninstalled (daemon autostart removed). Config left in ~/.summarize/daemon.json\n');
        return true;
    }
    if (sub === 'run') {
        const cfg = await readDaemonConfig({ env: envForRun });
