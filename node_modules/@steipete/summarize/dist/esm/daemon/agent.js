...
`;
export function buildAgentPromptHash(automationEnabled) {
...
function buildSystemPrompt({ pageUrl, pageTitle, pageContent, automationEnabled, }) {
...
function overrideModelBaseUrl(model, baseUrl) {
    if (!baseUrl)
        return model;
    return { ...model, baseUrl };
}
function resolveModelWithFallback({ provider, modelId, baseUrl, }) {
    try {
        return overrideModelBaseUrl(getModel(provider, modelId), baseUrl);
    }
    catch (error) {
        if (baseUrl) {
...
async function resolveAgentModel({ env, pageContent, modelOverride, }) {
    ...
    const applyBaseUrlOverride = (provider, modelId) => {
...
export async function completeAgentResponse({ env, pageUrl, pageTitle, pageContent, messages, modelOverride, tools, automationEnabled, }) {
    const normalizedMessages = normalizeMessages(messages);
    const toolList = automationEnabled
        ? tools
            .map((toolName) => TOOL_DEFINITIONS[toolName])
