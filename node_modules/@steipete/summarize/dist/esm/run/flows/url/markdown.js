...
import { createTranscriptToMarkdownConverter, } from '../../../llm/transcript-to-markdown.js';
...
export function createMarkdownConverters(ctx, options) {
    ...
    const wantsTranscriptMarkdown = ctx.flags.format === 'markdown' &&
    ...
    const effectiveMarkdownMode = markdownRequested || transcriptMarkdownRequested ? ctx.flags.markdownMode : 'off';
    const markdownModel = (() => {
        ...
        if (ctx.model.requestedModel.kind === 'fixed' &&
    ...
    const llmHtmlToMarkdown = markdownRequested &&
        ...
        ? createHtmlToMarkdownConverter({
            ...
            openaiBaseUrlOverride: markdownModel.openaiBaseUrlOverride ?? ctx.model.apiStatus.providerBaseUrls.openai,
        ...
        : null;
    const markitdownHtmlToMarkdown = markdownRequested && ctx.flags.preprocessMode !== 'off' && hasUvxCli(ctx.io.env)
        ? async (args) => {
            void args.url;
            void args.title;
            void args.siteName;
    ...
    const convertHtmlToMarkdown = markdownRequested
        ? async (args) => {
            ...
            if (ctx.flags.extractMode) {
                ...
                throw new Error('No HTML→Markdown converter configured (install uvx/markitdown or use --markdown-mode llm)');
            }
            if (llmHtmlToMarkdown) {
                try {
    ...
    const convertTranscriptToMarkdown = transcriptMarkdownRequested && markdownModel !== null
