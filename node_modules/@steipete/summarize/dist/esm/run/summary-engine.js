...
import { createStreamOutputGate } from './stream-output.js';
import { canStream, isGoogleStreamingUnsupportedError, isStreamingTimeoutError, mergeStreamingChunk, } from './streaming.js';
import { resolveModelIdForLlmCall, summarizeWithModelId } from './summary-llm.js';
import { isRichTty, markdownRenderWidth, supportsColor } from './terminal.js';
export function createSummaryEngine(deps) {
    const applyZaiOverrides = (attempt) => {
        if (!attempt.userModelId.toLowerCase().startsWith('zai/'))
            return attempt;
        return {
            ...attempt,
    ...
    const runSummaryAttempt = async ({ attempt, prompt, allowStreaming, onModelChosen, cli, streamHandler, }) => {
        ...
        if (modelResolution.note && deps.verbose) {
            writeVerbose(deps.stderr, deps.verbose, modelResolution.note, deps.verboseColor, deps.envForRun);
        }
        const parsedModelEffective = parseGatewayStyleModelId(modelResolution.modelId);
        const streamingEnabledForCall = allowStreaming &&
        ...
        const forceChatCompletions = Boolean(attempt.forceChatCompletions) ||
            (deps.openaiUseChatCompletions && parsedModelEffective.provider === 'openai');
        const maxOutputTokensForCall = await deps.resolveMaxOutputTokensForCall(parsedModelEffective.canonical);
        const maxInputTokensForCall = await deps.resolveMaxInputTokensForCall(parsedModelEffective.canonical);
        if (typeof maxInputTokensForCall === 'number' &&
            ...
            if (tokenCount > maxInputTokensForCall) {
                throw new Error(`Input token count (${formatCompactCount(tokenCount)}) exceeds model input limit (${formatCompactCount(maxInputTokensForCall)}). Tokenized with GPT tokenizer; prompt included.`);
            }
        }
        ...
        const shouldStreamSummaryToStdout = streamingEnabledForCall && !shouldRenderMarkdownToAnsi && !hasStreamHandler;
        const shouldStreamRenderedMarkdownToStdout = streamingEnabledForCall && shouldRenderMarkdownToAnsi && !hasStreamHandler;
        ...
        try {
            ...
            if (isStreamingTimeoutError(error)) {
                writeVerbose(deps.stderr, deps.verbose, `Streaming timed out for ${parsedModelEffective.canonical}; falling back to non-streaming.`, deps.verboseColor, deps.envForRun);
                const result = await summarizeWithModelId({
                    modelId: parsedModelEffective.canonical,
                    prompt,
                    maxOutputTokens: maxOutputTokensForCall ?? undefined,
                ...
                writeVerbose(deps.stderr, deps.verbose, `Google model ${parsedModelEffective.canonical} rejected streamGenerateContent; falling back to non-streaming.`, deps.verboseColor, deps.envForRun);
                const result = await summarizeWithModelId({
                    modelId: parsedModelEffective.canonical,
                    prompt,
                    maxOutputTokens: maxOutputTokensForCall ?? undefined,
        ...
        if (streamResult) {
            ...
            const streamer = shouldStreamRenderedMarkdownToStdout
                ? createMarkdownStreamer({
                    render: (markdown) => renderMarkdownAnsi(prepareMarkdownForTerminalStreaming(markdown), {
                        width: liveWidth,
                        wrap: true,
