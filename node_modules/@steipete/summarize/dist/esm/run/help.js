...
import { CLI_THEME_NAMES, createThemeRenderer, resolveThemeNameFromSources, resolveTrueColor, } from '../tty/theme.js';
...
export function buildProgram() {
    return new Command()
        ...
        .option('--youtube <mode>', 'YouTube transcript source: auto, web, no-auto (skip auto-generated captions), yt-dlp, apify', 'auto')
        .addOption(new Option('--transcriber <name>', 'Audio transcription backend: auto (default), whisper, parakeet, canary').choices(['auto', 'whisper', 'parakeet', 'canary']))
        ...
        .default('auto'))
        .option('--slides [value]', 'Extract slides for YouTube/direct video URLs and render them inline inside the summary narrative (when supported). Combine with --extract to interleave slides in the full transcript.')
        ...
        .option('--slides-scene-threshold <value>', 'Scene detection threshold for slide changes (0.1-1.0).', '0.3')
        ...
        .option('--format <format>', 'Website/file content format: md|text. For websites: controls the extraction format. For files: controls whether we try to preprocess to Markdown for model compatibility. (default: text; default in --extract mode for URLs: md)', undefined)
        .addOption(new Option('--preprocess <mode>', 'Preprocess inputs for model compatibility: off, auto (fallback), always.')
        .choices(['off', 'auto', 'always'])
        .default('auto'))
        .addOption(new Option('--markdown-mode <mode>', 'Markdown conversion: off, auto, llm (force LLM), readability. For websites: converts HTML→Markdown. For YouTube/transcripts: llm mode formats raw transcripts into clean markdown with headings and paragraphs.').default('readability'))
        .addOption(new Option('--markdown <mode>', 'Deprecated alias for --markdown-mode (use --extract --format md --markdown-mode ...)').hideHelp())
        .option('--length <length>', 'Summary length: short|medium|long|xl|xxl (or s/m/l) or a character limit like 20000, 20k', 'xl')
        .option('--max-extract-characters <count>', 'Maximum characters to print in --extract (default: unlimited).', undefined)
        .option('--language, --lang <language>', 'Output language: auto (match source), en, de, english, german, ... (default: auto; configurable in ~/.summarize/config.json via output.language)', undefined)
        .option('--max-output-tokens <count>', 'Hard cap for LLM output tokens (e.g. 2000, 2k). Overrides provider defaults.', undefined)
        .option('--force-summary', 'Force LLM summary even when extracted content is shorter than the requested length.', false)
        ...
        .option('--model <model>', 'LLM model id: auto, <name>, cli/<provider>/<model>, xai/..., openai/..., google/..., anthropic/..., zai/... or openrouter/<author>/<slug> (default: auto)', undefined)
        .option('--prompt <text>', 'Override the summary prompt (instruction prefix; context/content still appended).', undefined)
        ...
        .addOption(new Option('--theme <name>', `CLI theme (${CLI_THEME_NAMES.join(', ')})`).choices(CLI_THEME_NAMES))
...
export function applyHelpStyle(program, env, stdout) {
...
export function buildSlidesProgram() {
    return new Command()
        ...
        .option('--slides-max <count>', 'Maximum slides to extract (default: 6).', '6')
        .option('--slides-min-duration <seconds>', 'Minimum seconds between slides (default: 2).', '2')
        .addOption(new Option('--render <mode>', 'Inline render mode: auto, kitty, iterm, none.')
...
export function attachRichHelp(program, env, stdout) {
    ...
    const heading = (text) => theme.heading(text);
    const cmd = (text) => theme.accentStrong(text);
    const dim = (text) => theme.dim(text);
...
export function buildConciseHelp() {
    return [
...
export function buildRefreshFreeHelp() {
    return [
...
export function buildDaemonHelp() {
    return [
...
export function buildTranscriberHelp() {
    return [
