...
function isProbablyUnstableGoogleModelId(modelId) {
...
async function listGoogleModels({ apiKey, fetchImpl, timeoutMs, }) {
...
function pickSuggestions(models, limit) {
    const prefer = (s) => s.toLowerCase();
    const ids = models
        ...
        const aa = prefer(a);
        const bb = prefer(b);
        const score = (id) => (id.includes('flash') ? 0 : 10) +
...
export async function resolveGoogleModelForUsage({ requestedModelId, apiKey, fetchImpl, timeoutMs, }) {
    ...
    try {
        ...
        throw new Error(`Cannot verify Google model availability for ${requestedModelId}. ${message}\n` +
            `Check GEMINI_API_KEY (or GOOGLE_GENERATIVE_AI_API_KEY / GOOGLE_API_KEY) and that the Gemini API is enabled for this key.`, { cause: error });
    }
    const byId = new Map();
    ...
    throw new Error(`Google model ${requestedModelId} is not available via the Gemini API (v1beta) for this API key. ${hint}`);
}
