...
function guessExtension({ filenameHint, mediaType, }) {
...
export async function convertToMarkdownWithMarkitdown({ bytes, filenameHint, mediaTypeHint, uvxCommand, timeoutMs, env, execFileImpl, }) {
    ...
    try {
        ...
        const { stdout } = await execFileText(execFileImpl, uvxCommand && uvxCommand.trim().length > 0 ? uvxCommand.trim() : 'uvx', ['--from', from, 'markitdown', filePath], {
            timeout: timeoutMs,
            env: { ...process.env, ...env },
