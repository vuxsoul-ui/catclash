...
export async function transcribeMediaWithWhisper({ bytes, mediaType, filename, openaiApiKey, falApiKey, totalDurationSeconds = null, onProgress, env = process.env, }) {
    const notes = [];
...
export async function transcribeMediaFileWithWhisper({ filePath, mediaType, filename, openaiApiKey, falApiKey, segmentSeconds = DEFAULT_SEGMENT_SECONDS, totalDurationSeconds = null, onProgress = null, env = process.env, }) {
    const notes = [];
    const onnxPreference = resolveOnnxModelPreference(env);
    if (onnxPreference) {
        onProgress?.({
