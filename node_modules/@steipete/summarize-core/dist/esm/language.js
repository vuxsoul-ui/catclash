...
const looksLikeLanguageTag = (value) => 
...
function normalizeLanguageTag(value) {
    ...
    const tail = rest.map((p) => p.length === 2 ? p.toUpperCase() : p.slice(0, 1).toUpperCase() + p.slice(1));
    return [head, ...tail].join('-');
}
function sanitizeFreeForm(value) {
    const trimmed = value.trim();
    if (!trimmed)
        return '';
    return trimmed.replaceAll(/\s+/g, ' ').slice(0, 64);
}
export function parseOutputLanguage(raw) {
    ...
    if (alias)
        return { kind: 'fixed', tag: alias.tag, label: alias.label };
...
export function resolveOutputLanguage(raw) {
...
export function formatOutputLanguageInstruction(language) {
...
export function formatOutputLanguageForJson(language) {
