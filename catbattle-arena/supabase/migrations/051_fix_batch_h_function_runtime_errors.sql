-- Fix remaining active production function errors (Batch H).
-- - public.open_crate: uuid/text mismatch in crate predicate
-- - public.reroll_cat_stats: direct dependency on missing columns
-- - public.resolve_match: remove non-existent cats.updated_at reference and use tolerant stat updates
...
DECLARE
  ...
  v_result JSONB;
BEGIN
  v_user := NULLIF(BTRIM(p_user_id), '')::uuid;
  IF v_user IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'error', 'invalid_user_id');
  END IF;

  -- Preserve legacy-safe behavior without hard-binding to optional crate history tables.
  ...
  -- If cosmetic exists and user doesn't own it, grant cosmetic; otherwise grant xp fallback.
  IF v_rarity IN ('Common', 'Rare', 'Epic', 'Legendary') THEN
    ...
    IF v_cosmetic.id IS NOT NULL THEN
      ...
      VALUES (v_user, v_cosmetic.id, p_crate_type)
...
DECLARE
  ...
  v_max INTEGER;
BEGIN
  -- Conservative behavior in schemas without sigils: use xp as the payment fallback currency.
